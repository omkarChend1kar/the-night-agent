...
# --rm removed from command because we want to name it and manage it. Or keep it?
# User supplied: docker run --pull=always --rm -it ...
# I convert to: docker run -d --pull=always --name kestra-local ...
...
#   -v /var/run/docker.sock:/var/run/docker.sock \
#   -v /tmp:/tmp \
#   -v "$PROJECT_ROOT/backend/kestra-config.yml":/app/kestra-config.yml \
#   -v "$PROJECT_ROOT/backend":/app/backend \
#   kestra/kestra:latest server local --config /app/kestra-config.yml >/dev/null
